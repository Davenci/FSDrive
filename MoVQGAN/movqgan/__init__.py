...
def get_movqgan_model(name, pretrained=True, device='cuda', cache_dir='movqgan_ckpt', **model_kwargs):
    ...
    if pretrained:
        ...
        # )
        checkpoint = torch.load("movqgan_ckpt/movqgan_270M.ckpt",weights_only=True)
        # checkpoint = torch.load(os.path.joid(cache_dir, config['filename']))ddddlllaa
