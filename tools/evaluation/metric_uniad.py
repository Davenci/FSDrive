...
# CHANGED: Updated import for PyTorch Lightning 2.x compatibility
# OLD: from pytorch_lightning.metrics.metric import Metric
# NEW: torchmetrics.Metric is now a separate package in PyTorch Lightning 2.x
...
class PlanningMetric(Metric):
    def __init__(
        self,
        n_future=6,
        # CHANGED: Removed compute_on_step parameter for torchmetrics compatibility
        # OLD: compute_on_step: bool = False,
        # NEW: This parameter was removed in torchmetrics (no longer needed)
    ):
        # CHANGED: Removed compute_on_step argument from super().__init__()
        # OLD: super().__init__(compute_on_step=compute_on_step)
        # NEW: super().__init__() - compute_on_step parameter no longer exists in torchmetrics
        super().__init__()
